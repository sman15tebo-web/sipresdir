...
async function loadJenisKonsekuensi() {
    ...
    try {
        ...
        jenisKonsekuensiList = Array.isArray(res.data) ? res.data : [];
        renderJenisKonsekuensiTable();
    } catch (error) {
        tbody.innerHTML = '<tr><td colspan="4" class="text-center py-8 text-red-500">Gagal memuat: ' + (error.message || error) + '</td></tr>';
    }
}
...
// ---------- TAB 2: PETAKAN ----------
function getTanggalHariIniKons() {
    const now = new Date();
    return `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`;
}

function pilihTanggalHariIniKons() {
    const dateInput = document.getElementById('konsFilterDate');
    if (dateInput) dateInput.value = getTanggalHariIniKons();
    loadSiswaTerlambatKons();
}
...
async function loadPetakanKonsekuensi() {
    ...
    const dateInput = document.getElementById('konsFilterDate');
    if (dateInput && !dateInput.value) dateInput.value = getTanggalHariIniKons();
...
async function loadSiswaTerlambatKons() {
    ...
    const dateInput = document.getElementById('konsFilterDate');
    const dateStr = dateInput && dateInput.value ? dateInput.value : getTanggalHariIniKons();
...
async function loadChecklistKonsekuensi() {
    ...
    let res;
    try { res = await fetchAPI('getJenisKonsekuensi', { token: currentUser.token }); }
    catch (error) { container.innerHTML = '<p class="text-red-500 text-sm">Gagal memuat: ' + (error.message || error) + '</p>'; return; }
    if (!res.success) { container.innerHTML = '<p class="text-red-500 text-sm">Gagal memuat: ' + res.message + '</p>'; return; }
    jenisKonsekuensiList = Array.isArray(res.data) ? res.data : [];
...
function resetSusunanKonsekuensi() {
    const checked = Array.from(document.querySelectorAll('[id^="checkKons_"]:checked')).map(function(cb) { return cb.value; });
    if (!checked.length) {
        hasilGenerateKonsekuensi = [];
        document.getElementById('hasilGenerateKonsekuensi').innerHTML = '';
        Swal.fire({ icon: 'info', title: 'Belum ada pilihan', text: 'Pilih jenis konsekuensi terlebih dahulu.', timer: 1800, showConfirmButton: false });
        return;
    }

    hasilGenerateKonsekuensi = jenisKonsekuensiList
        .filter(function(k) { return checked.includes(k.id); })
        .map(function(k) { return Object.assign({}, k, { siswa: [] }); });
    renderHasilGenerate();
    Swal.fire({ icon: 'success', title: 'Susunan direset', text: 'Semua pembagian siswa kembali menjadi 0.', timer: 1600, showConfirmButton: false });
}
...
function renderHasilGenerate() {
    ...
    container.innerHTML = '<div class="mt-6 pt-6 border-t border-gray-200"><div class="flex flex-wrap items-center justify-between gap-3 mb-4"><h4 class="text-base font-bold text-gray-700 flex items-center gap-2"><i class="fas fa-check-circle text-green-500"></i> Hasil Generate Konsekuensi</h4><button onclick="simpanRiwayatKonsekuensi()" class="bg-emerald-600 text-white hover:bg-emerald-700 px-4 py-2 rounded-xl text-sm font-bold shadow-md transition flex items-center gap-2"><i class="fas fa-save"></i> Simpan Penugasan</button></div><div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">' + cards + '</div></div>';
}

async function simpanRiwayatKonsekuensi() {
    const dateInput = document.getElementById('konsFilterDate');
    const tanggal = dateInput && dateInput.value ? dateInput.value : new Date().toLocaleDateString('en-CA');
    const assignments = [];
    hasilGenerateKonsekuensi.forEach(function(k) {
        k.siswa.forEach(function(s) {
            assignments.push({ nisn: s.nisn, nama: s.nama, kelas: s.kelas, idKonsekuensi: k.id, namaKonsekuensi: k.nama });
        });
    });
    if (!assignments.length) { Swal.fire('Info', 'Belum ada siswa yang dipetakan.', 'info'); return; }
    const res = await fetchAPI('saveRiwayatKonsekuensi', { token: currentUser.token, tanggal, assignments });
    if (res.success) {
